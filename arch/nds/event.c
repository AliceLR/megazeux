...
void platform_init_event(void)
{
  ...
  // Safety binding for if pad.config fails to load.
  if(input.joystick_global_map.button[0][5] == 0)
  {
    input.joystick_global_map.button[0][5] =
     input.joystick_global_map.show_screen_keyboard_action[0];
  }
}
