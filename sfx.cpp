...
int note_freq[12]={//Frequencies of 6C thru 6B
	2032,2152,2280,2416,2560,2712,2880,3048,3232,3424,3624,3840 };
int sam_freq[12]={//Sample frequencies of 0C (C-~1) thru 0B (B-~1)
	3424,3232,3048,2880,2712,2560,2416,2280,2152,2032,1920,1812 };

noise far background[NOISEMAX];	//The sound queue itself
...
void play_sfx(int sfxn) {
	if(sfxn>=NUM_SFX) return;
	if(custom_sfx_on) play_str(&custom_sfx[sfxn*69],1);
	else play_str(sfx_strs[sfxn],1);
}

//sfx_play=1 to NOT play non-digi unless queue empty
void play_str(unsigned char far *str,char sfx_play) {
	int t1,oct=3,note=1,dur=18,t2,last_note=-1,digi_st=-1,digi_end;
	unsigned char chr;
	char nn[7]={ 10,12,1,3,5,6,8 };//Note trans. table from 1-7 (a-z) to 1-12
	if(!sfx_on) sfx_play=1;//SFX off
	else if(!sound_in_queue) sfx_play=0;
	//Now, if sfx_play, only play digi
	for(t1=0;t1<str_len(str);t1++) {
		chr=str[t1];
		...
		if(chr=='X') {//Rest
			if(!sfx_play) submit_sound(F_REST,dur);
			continue;
			}
		if((chr>47)&&(chr<55)) oct=chr-48;//Set octave
		if(chr=='.') dur+=(dur>>1);//Dot (duration 150%)
		if(chr=='!') dur/=3;//Triplet (cut duration to 33%)
		if((chr>64)&&(chr<72)) {//Note
			note=nn[chr-65];//Convert to 1-12
			t2=oct;//Save old octave in case # or $ changes it
			if(str[t1+1]=='#') {//Increase if sharp
				t1++;
				if(++note==13) {//"B#":="+C-"
					note=1;
					if(++oct==7) oct=6;
			...
			else if(str[t1+1]=='$') {//Decrease if flat
				t1++;
				if(--note==0) {//"C$":="-B+"
					note=12;
					if(--oct==-1) oct=0;
		...
		if(chr=='Z') dur=9;//64th notes
		if(chr=='T') dur=18;//32nd notes (default)
		if(chr=='S') dur=36;//16th
		if(chr=='I') dur=72;//8th
		if(chr=='Q') dur=144;//1/4
		if(chr=='H') dur=288;//1/2
		if(chr=='W') dur=576;//Whole
		if(chr=='&') {
			//Digitized
			digi_st=++t1;
			if(str[t1]==0) break;
		...
		if(chr=='_') {
			digi_st=-1;
			if((music_on)&&(music_device<6)) break;
			}
		}
}
...
void submit_sound(int freq,int delay) {
  ...
  if(topindex!=(backindex-1)) {//Queue full?
	...
	if(backduration) backduration--;//Sound is in progress
		...
		if(sound_in_queue) {//Current sound finished, more?
			...
			if(background[backindex].freq==F_REST) {
				if(sfx_on) nosound();//freq==1 for rest
				}
			else if(sfx_on) sound(background[backindex].freq);//Start sound
			backindex++;
		...
		else if(sfx_on) nosound();
		}
}

unsigned char far *sfx_strs[NUM_SFX]={
"5c-gec-gec", // Gem
...
unsigned char far *custom_sfx=NULL;
char custom_sfx_on=0;//1 to turn on custom sfx

char sfx_init(void) {
	custom_sfx=(unsigned char *)farmalloc(NUM_SFX*69);
	if(custom_sfx==NULL) return 1;
	return 0;
}

void sfx_exit(void) {
	if(custom_sfx!=NULL) farfree(custom_sfx);
	custom_sfx=NULL;
}
