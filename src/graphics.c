...
static void resize_window(unsigned window_id, boolean fullscreen)
{
  if(graphics.renderer.resize_window)
  {
    ...
    // FIXME: may have to recreate the window here because SDL has issues
    // switching to/from fullscreen windowed it seems
    graphics.renderer.resize_window(&graphics, &graphics.window);
    //graphics.renderer.create_window(&graphics, &graphics.window);
