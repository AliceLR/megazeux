 ...
 const struct function_counter *counter, const char *name, int value, int id)
{
  ...
  {  // Old (somewhat dodgy) behaviour tied to <2.85
    value %= 3;
    switch(value)
    {
      case 0:
  ...
  { // 2.85 makes use of both flags to allow 3 different ccheck modes
    value %= 4;
    cur_sprite->flags &= ~(SPRITE_CHAR_CHECK | SPRITE_CHAR_CHECK2);
    switch(value)
    {
...
{
  int spr_num = strtol(name + 3, NULL, 10) & (MAX_SPRITES - 1);
  if (mzx_world->version < 0x0255) // Before 2.85 these fields were ints.
...
static const struct function_counter builtin_counters[] =
{
  ...
  { "smzx_idx!,!", 0x0255, smzx_idx_read, smzx_idx_write },              // 2.85
  ...
  { "spr!_offset", 0x0255, spr_offset_read, spr_offset_write },      // 2.85
  ...
  { "spr!_tcol", 0x0255, spr_tcol_read, spr_tcol_write },            // 2.85
  { "spr!_unbound", 0x0255, spr_unbound_read, spr_unbound_write },   // 2.85
